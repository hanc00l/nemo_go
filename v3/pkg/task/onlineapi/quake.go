...
func (q *Quake) GetRequiredResources() (re []core.RequiredResource) {
	//CDN
...
func (q *Quake) GetQueryString(domain string, config execute.OnlineAPIConfig, filterKeyword map[string]struct{}) (query string) {
	...
	if config.IsIgnoreChinaOther {
		...
		if config.IsIgnoreOutsideChina {
			query = fmt.Sprintf("(%s) AND country:\"CN\"", query)
		}
	}
	return
}

func (q *Quake) Run(query string, apiKey string, pageIndex int, pageSize int, config execute.OnlineAPIConfig) (pageResult []OnlineSearchResult, sizeTotal int, err error) {
	data := quakePostData{
		Query:       query,
		Latest:      true,
		IgnoreCache: true,
	...
	resp, err := utils.GetProxyHttpClient(q.IsProxy).Do(request)
	if err != nil {
		return
	}
	if resp.Body != nil {
		...
		if strings.Contains(string(body), "/quake/login") {
			return nil, 0, errors.New("quake的token不正确")
		}
...
func (q *Quake) parseQuakeSearchResult(queryResult []byte) (result []OnlineSearchResult, finish bool, sizeTotal int) {
	...
	if err != nil {
		...
		logging.RuntimeLog.Errorf("Quake搜索失败:%s", err)
		return
	}
	if strings.HasPrefix(serviceInfo.Message, "Successful") == false {
		logging.RuntimeLog.Errorf("Quake搜索失败:%s", serviceInfo.Message)
		return
	}
	for _, data := range serviceInfo.Data {
		...
		if len(data.Service.HTTP.Host) > 0 {
			qsr.Host = data.Service.HTTP.Host
			if !utils.CheckIP(data.Service.HTTP.Host) {
				qsr.Domain = data.Service.HTTP.Host
			}
		} else {
			qsr.Host = data.IP
		}
